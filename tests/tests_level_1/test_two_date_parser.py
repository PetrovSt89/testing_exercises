...
def test__compose_datetime_from__empty_date_str(date_now):
    assert compose_datetime_from(
        date_str='',
        time_str=date_now.time().strftime("%H:%M")
        ) == datetime.datetime.strptime(str_datetime_now, "%y-%m-%d %H:%M")

def test__compose_datetime_from__date_str_is_tomorrow(date_now):
    assert compose_datetime_from(
        date_str="tomorrow",
        time_str=date_now.time().strftime("%H:%M")
